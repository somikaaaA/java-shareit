...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findByUserIdCurrentBook(@Param("userId") Long userId,
                                          @Param("currentDate") LocalDateTime currentDate);

    @Query("SELECT b FROM Booking b WHERE (b.booker.id = :userId OR b.item.owner.id = :userId) " +
            "AND b.end < :currentDate " +
            "ORDER BY b.start DESC")
    List<Booking> findByUserIdPastBook(@Param("userId") Long userId,
                                       @Param("currentDate") LocalDateTime currentDate);

    @Query("SELECT b FROM Booking b WHERE (b.booker.id = :userId OR b.item.owner.id = :userId) " +
            "AND b.start > :currentDate " +
            "ORDER BY b.start DESC")
    List<Booking> findByUserIdFutureBook(@Param("userId") Long userId,
                                         @Param("currentDate") LocalDateTime currentDate);
    ...
    @Query("SELECT b FROM Booking b WHERE b.item.id = :itemId " +
            "AND b.start < :currentDate AND b.end > :currentDate ")
    Optional<Booking> findByItemIdCurrentBook(@Param("itemId") Long itemId,
                                              @Param("currentDate") LocalDateTime currentDate);
